...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "clearIcons") {
        ...
        const settings = document.querySelector(".ytp-settings-button");
        const miniplayer = document.querySelector(".ytp-miniplayer-button");
        [play, skip, volume, autoplay, cc, settings, miniplayer].forEach((btn) => {
            if (!btn) return;
...
function clearIconsFromDOM2() {
    document
        .querySelectorAll(
            ".ytp-miniplayer-button i, .ytp-play-button i, .ytp-settings-button i,.ytp-subtitles-button i, .ytp-next-button i, .ytp-volume-icon i, .ytp-autonav-toggle i, .ytp-play-button img, .ytp-next-button img, .ytp-volume-icon img, .ytp-autonav-toggle img,"
        )
